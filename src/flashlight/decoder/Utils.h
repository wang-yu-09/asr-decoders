                      ...
                      const bool returnSorted) 
{
  ...
  {
    if (candidatePtrs[i]->compareNoScoreStates(candidatePtrs[nHypAfterMerging - 1]) != 0) 
    {
      // Distinct candidate
    ...
    {
      // Same candidate
      double maxScore = std::max(candidatePtrs[nHypAfterMerging - 1]->score, candidatePtrs[i]->score);
      if (logAdd) 
      {
