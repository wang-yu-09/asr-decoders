...
void LexiconDecoder::decodeBegin() 
{
	...
	hyp_.emplace(0, std::vector<LexiconDecoderState>());
 	...
 	hyp_[0].emplace_back( 
		 					0.0, //总得分
							lm_->start(0), // 设置kenlm语音模型初始状态
							lexicon_->getRoot(), // 指向词典树的root节点
							nullptr, // 父状态指针
							sil_, // token id
							-1 // word id, -1 代表不是一个完整单词
						); 
...
void LexiconDecoder::decodeStep(const float* emissions, int T, int N) 
{
  	...
  	for (int t = 0; t < T; t++) 
	{
		...
		{
			const TrieNode* prevLex = prevHyp.lex; // root
			const int prevIdx = prevHyp.token; // sil_
			const float lexMaxScore = ( prevLex == lexicon_->getRoot() ? 0 : prevLex->maxScore );

			/* (1) Try children */
			for (int r = 0; r < std::min(opt_.beamSizeToken, N); ++r) 
			{
				int n = idx[r]; // class id
				// 如果词典树上不能到达这个节点, 抛弃, 只保留可以组合成单词的节点
				...
				/*
				if (nDecodedFrames_ + t > 0 && opt_.criterionType == CriterionType::ASG) 
				{
				...
				*/
				...
				if ( prevHyp.prevBlank || n != prevIdx ) 
				{
					// 如果这个节点还有子节点
			...
			// if ( opt_.criterionType != CriterionType::CTC || !prevHyp.prevBlank || prevLex == lexicon_->getRoot()) 
			if ( ! prevHyp.prevBlank || prevLex == lexicon_->getRoot() )
			{
...
void LexiconDecoder::decodeEnd() 
{
  	...
  	for (const LexiconDecoderState& prevHyp : hyp_[nDecodedFrames_ - nPrunedFrames_]) 
	{
